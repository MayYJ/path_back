    ...
    <el-table :data="tableData" style="width: 1000px;margin:0px auto;">
      ...
      <el-table-column label="准备数据" width="140">
        <template slot-scope="scope">
          <span
            ...
            class="mouseIn"
            :class="{'waitToChoose': scope.row.prepareDataState === '进行准备'}"
      ...
      <el-table-column label="查看结果" width="140">
        <template slot-scope="scope">
          <span
            class="mouseIn waitToChoose"
            @click="theResult(scope.$index, scope.row)"
          >{{ scope.row.resultsState }}</span>
        </template>
      </el-table-column>
        ...
        .then(function(response) {
          ...
          if (response.data.status == 0) {
            that.$notify({
              title: "警告",
              message:
                "项目*" + that.tableData[index].questionName + "*删除成功",
              type: "warning"
            });
