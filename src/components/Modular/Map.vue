  ...
  <div class="mapBox">
    ...
    <div class="pointList">
      ...
      <div class="updataPoint">
        <el-button type="warning" @click="updataPoint">
          上传所有点
...
export default {
  ...
  methods: {
    ...
    fristMap(res) {
      ...
      // map.centerAndZoom(new BMap.Point(66.404, 39.915), 18) // 初始化地图,设置中心点坐标和地图级别
      map.addControl(
        ...
        .then(res => {
          if (res.data.status === 0) {
            ...
            if (res.data.object.length !== 0) {
              ...
              map.centerAndZoom(
                new BMap.Point(res.data.object[0].lng, res.data.object[0].lat),
                15
              ) // 初始化地图,设置中心点坐标和地图级别
              for (let i in res.data.object) {
                var m1 = new BMap.Marker(
                  new BMap.Point(res.data.object[i].lng, res.data.object[i].lat)
                ) // 创建3个marker
                map.addOverlay(m1)
                var lab1 = new BMap.Label(res.data.object[i].nodeAddress, {
                  position: new BMap.Point(
                    res.data.object[i].lng,
                    res.data.object[i].lat
                  )
                }) // 创建3个label
                map.addOverlay(lab1)
              }
              ...
              map.centerAndZoom(new BMap.Point(106.61461768, 29.5382169), 15) // 初始化地图,设置中心点坐标和地图级别
              return false
            }
          } else {
            map.centerAndZoom(new BMap.Point(106.61461768, 29.5382169), 15) // 初始化地图,设置中心点坐标和地图级别
